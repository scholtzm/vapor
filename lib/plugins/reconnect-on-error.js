    ...
    VaporAPI.registerHandler({
        ...
        function(error) {
            switch (error.cause) {
                ...
                case 'loginError':
                    if(error.response.eresult === Steam.EResult.ServiceUnavailable ||
                    error.response.eresult === Steam.EResult.TryAnotherCM) {
                        log.info('Reconnecting in %dms.', timeout);
                        setTimeout(VaporAPI.connect, timeout);
                    }
                    ...
                    break;
                default:
                    throw new Error('Unknown error cause: %s.', error.cause);
            }
        }
    );
};
