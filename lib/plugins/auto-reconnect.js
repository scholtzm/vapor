            ...
            switch(error.eresult) {
                ...
                case Steam.EResult.TryAnotherCM:
                    VaporAPI.emitEvent('message:info',
                        'Reason: ' + utils.enumToString(error.eresult, Steam.EResult) + '. Reconnecting in ' + timeout + 'ms.');
                    setTimeout(function() {
                        VaporAPI.connect();
                    }, timeout);
                    break;
                ...
                default:
                    VaporAPI.emitEvent('message:warn', 'Reason of disconnect: ' + utils.enumToString(error.eresult, Steam.EResult));
                    break;
            }
        }
    );
