 ...
 * `steamFriends`, `steamTrading`, `steamGameCoordinator`, `plugin` or `*` (which stands for 'any').
...
API.prototype.hasHandler = function(options) {
    ...
    switch(options.emitter) {
        ...
        case 'client':
            return vapor.client.listeners(options.event).length > 0;

        case 'steamUser':
            return vapor.steamUser.listeners(options.event).length > 0;

        case 'steamFriends':
            return vapor.steamFriends.listeners(options.event).length > 0;

        case 'steamTrading':
            return vapor.steamTrading.listeners(options.event).length > 0;

        case 'steamGameCoordinator':
            return vapor.steamGameCoordinator.listeners(options.event).length > 0;
        ...
        case GLOBAL_EVENT_EMITTER:
            ...
            total += vapor.client.listeners(options.event).length;
            total += vapor.steamUser.listeners(options.event).length;
            total += vapor.steamFriends.listeners(options.event).length;
            total += vapor.steamTrading.listeners(options.event).length;
            total += vapor.steamGameCoordinator.listeners(options.event).length;
