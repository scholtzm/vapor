...
Utils.prototype.isAdmin = function(steamID) {
    return !!~this._vapor.config.admins.indexOf(steamID);
};
...
Utils.prototype.getUserDescription = function(steamID, format) {
    ...
    if(steamID in this._vapor.steamFriends.personaStates) {
        result = result.replace('%username', this._vapor.steamFriends.personaStates[steamID].player_name);
    } else {
...
Utils.prototype.getTimestamp = function(unixTimestamp, format) {
    return moment.unix(unixTimestamp).format(format || this._vapor.config.logs.dateFormat);
};
