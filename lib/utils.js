...
Utils.prototype.getUserDescription = function(steamID, format) {
    format = format || '%username (%steamid)';
    var result = format;

    // Get %username if possible
    if(steamID in _vapor.steamFriends.personaStates) {
        result = result.replace('%username', _vapor.steamFriends.personaStates[steamID].player_name);
    } else {
...
Utils.prototype.getTimestamp = function(unixTimestamp, format) {
    return moment.unix(unixTimestamp).format(format || _vapor.config.logs.dateFormat || 'YYYY-MM-DD HH:mm:ss');
};
