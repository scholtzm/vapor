 ...
 * @param  {string}   pluginName Plugin's unique identifier.
 * @param  {function} plugin     Main plugin function.
 * @param  {*}        data       Extra data passed to VaporAPI. If you need multiple values, use object.
 */
Vapor.prototype.use = function(pluginName, plugin, data) {
    // Check arguments
    if(typeof pluginName !== 'string') {
        throw new Error('Provided argument "pluginName" is not type of string in "Vapor.use"');
    }
    ...
    // Do not allow 2 plugins to share the same pluginName
    if(!!~this.loadedPlugins.indexOf(pluginName)) {
        this.log.warn('Plugin %s is already loaded. Please check your settings.', pluginName);
        return;
    }

    // Call it
    plugin(Object.seal(new API(this, pluginName, data)));

    // Mark as loaded
    this.loadedPlugins.push(pluginName);

    this.log.info('"%s" has been loaded successfully.', pluginName);
};
