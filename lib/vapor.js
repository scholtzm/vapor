...
Vapor.prototype.init = function(config) {
    ...
    config = Misc.verifyConfig(config);
    ...
    // Setup paths
    this.serverListPath = path.join(config.dataDir, Misc.DATA_MISC_DIR, 'servers.json');
    this.sentryFilePath = path.join(config.dataDir, Misc.DATA_SENTRY_DIR, config.username + '.sentry');
    ...
    if(fs.existsSync(this.serverListPath)) {
        try {
            ...
            this.log.warn('Failed to load server list from cache. Skipping.');
            this.log.warn(error);
        }
    }
...
Vapor.prototype.disconnect = function() {
    this.log.info('Disconnecting from Steam network.');
    ...
    var req = https.request(options, function(res) {
        if (res.statusCode === 200) {
            res.on('data', function(chunk) {
                ...
                _this.emit('cookies', cookies, sessionid);
            });
        } else {
